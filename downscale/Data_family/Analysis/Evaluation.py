...
class EvaluationFromDict(Evaluation):
    ...
    def _group_metric_result_by_wind_category(metric_result, variable):
        ...
        print(f"5 m/s <= NWP wind speed < 10 m/s: {len(metric_result[f'group_{variable}_nwp'][filter_low & filter_high])} ")
        ...
        print(f"10 m/s <= NWP wind speed < 15 m/s: {len(metric_result[f'group_{variable}_nwp'][filter_low & filter_high])} ")
    ...
    def plot_heatmap(self, results, variables=["UV"], metrics=["abs_error"], topo_carac="alti", fontsize=15,
        ...
        for variable in variables:
            ...
            for metric in metrics:
                ...
                metric_result[f"diff_cnn_nwp_{metric}"] = np.abs(metric_result[metric]) - np.abs(metric_result[f"nwp_{metric}"])

                # Heatmap month vs hour
                piv_table = metric_result.pivot_table(index="month", columns="hour", values=metric, aggfunc='mean')
                plt.figure()
                ax = plt.gca()
                sns.heatmap(piv_table, ax=ax, cmap='coolwarm', norm=MidpointNormalize(midpoint=0, vmin=-0.20, vmax=0.20))
                ...
                plt.figure()
                ax = plt.gca()
                sns.heatmap(piv_table, ax=ax, cmap='coolwarm', norm=MidpointNormalize(midpoint=0, vmin=-0.03, vmax=0.03))
