...
# downscale = DwnscHelbig()
# mu_mnt = downscale.mu_helbig_map(mnt, dx=25)
# laplacian_mnt = downscale.laplacian_map(mnt, dx=25)

t0 = t()
#output_downscale = downscale.downscale_helbig(mnt, idx_x=None, idx_y=None, type_input="map", library="numba")
#print(t() - t0)
...
mu_mnt_idx = downscale.mu_helbig_idx(mnt, dx=25, idx_x=idx_x, idx_y=idx_y)
laplacian_mnt_idx = downscale.laplacian_idx(mnt, dx=25, idx_x=idx_x, idx_y=idx_y)
x_dsc_topo_idx = downscale.x_dsc_topo_helbig(mnt, dx=25, idx_x=idx_x, idx_y=idx_y, type_input="indexes")

downscale = DwnscHelbig()

mu_mnt = downscale.mu_helbig_map(mnt_small, dx=25)
laplacian_mnt = downscale.laplacian_map(mnt_small, dx=25)
x_dsc_topo = downscale.x_dsc_topo_helbig(mnt_small)
