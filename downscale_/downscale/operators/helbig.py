...
class MovingAverage(Slope):
    ...
    def mu_average_idx(self, mnt, y_left, y_right, x_left, x_right, dx, verbose=True):
        ...
        mu_idx = np.expand_dims(mu_idx) if mu_idx.ndim == 0 else mu_idx
        for index, (i1, j1, i2, j2) in enumerate(zip(y_left, y_right, x_left, x_right)):
            mu_map_i = self.mu_helbig_map(mnt[i1 - 1:j1 + 1 + 1, i2 - 1:j2 + 1 + 1], dx, verbose=verbose)
    ...
    @check_kwarg_dtype({"idx_x": np.int32,
                        "idx_y": np.int32})
...
class MovingStd(MovingAverage):
    ...
    def std_average_num(self, library, mnt, idx_x, idx_y, x_win, y_win):
        ...
        if library == "numba" and _numba:
            ...
            std_avg_flat = self.std_slicing_numpy_loop(mnt, y_left, y_right, x_left, x_right)
            library = "numpy"
    ...
    @check_kwarg_dtype({"idx_x": np.int32,
                        "idx_y": np.int32})
    def xsi_helbig_map(self, mnt, mu, idx_x=None, idx_y=None, x_win=69 // 2, y_win=79 // 2, library="numba"):
...
class SgpHelbig(MovingStd):
    ...
    def x_sgp_topo_helbig_idx(self, mnt, idx_x=None, idx_y=None, dx=25, L=2_000, library="tensorflow",
        ...
        xsi = self.xsi_helbig_map(mnt, mu, idx_x, idx_y,
                                  x_win=x_win, y_win=y_win, library=library)
...
class DwnscHelbig(SgpHelbig):
    ...
    def x_dsc_topo_helbig(self, mnt, dx=25, idx_x=None, idx_y=None, library="numba", verbose=True):
        ...
        idx_x, idx_y, type_input = self.detect_type_input(idx_x, idx_y)

        if type_input == "map":
            laplacian = self.laplacian_map(mnt, dx, library=library, helbig=True)
            mu = self.mu_helbig_map(mnt, dx, verbose=verbose)
        elif type_input == "indexes":
            idx_x = np.expand_dims(idx_x, axis=0) if idx_x.ndim == 0 else idx_x
            idx_y = np.expand_dims(idx_y, axis=0) if idx_y.ndim == 0 else idx_y
            idx_x, idx_y = change_several_dtype_if_required([idx_x, idx_y], [np.int32, np.int32])
