...
def create_prm(month_prediction=True):
    ...
    prm["GPU"] = True
    prm["horovod"] = True
    prm["results_name"] = "2022_03_22_swiss"
    ...
    # Scaling
    prm[
        "scaling_function"] = "Arctan_38_2_2"  # linear, Arctan_30_1, Arctan_30_2, Arctan_10_2, Arctan_20_2, Arctan_40_2, Arctan_50_2, Arctan_38_2_2
    ...
    prm["variable"] = ["W", "UV", "UVW", "UV_DIR_deg", "alpha_deg", "NWP_wind_speed", "exp_Wind", "acceleration_CNN",
                       "Z0"]
    ...
    prm["coords_domain_for_map_prediction"] = [937875, 6439125, 973375,
                                               6464125]  # [959000, 6462000, 960000, 6464000]  #[xmin, ymin, xmax, ymax]
    prm["method"] = "linear"
    ...
    # SURFEX
    prm["path_SURFEX"] = prm[
                             "data_path"] + "/SURFEX/scriptMNT.nc"  # Surfex grid on the Grandes Rousses domain (from Ange)
    prm["path_save_prediction_on_surfex_grid"] = prm[
                                                     "data_path"] + "/SURFEX/"  # Where to save predictions on SURFEX grid
    ...
    prm["QC_pkl"] = prm[
                        "QC_path"] + "qc_france_swiss_v0.pkl"  # Quality controled time series (where to save the file), qc_21_01_2022
    ...
    prm["path_fast_loading"] = prm[
                                   "data_path"] + "BDclim/QC/qc_france_swiss_v0.pkl"  # qc_59.pkl before Quality controled time series (where to load the file)
...
for station in ["LA MEIJE-NIVOSE"]:
    ...
    for index in range(len(list_years)-1):
        ...
        if enough_data_year & enough_data_year_after:
            ...
            vqc_france_swiss_v0.pklalues = np.asarray([value for value in range(360)])
